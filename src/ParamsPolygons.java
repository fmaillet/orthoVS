 ...
 * @author Fred
 */
public class ParamsPolygons extends javax.swing.JPanel {

    /**
     * Creates new form ParamsSaccade
     */
    public ParamsPolygons() {
        initComponents();
    ...
    private void initComponents() {
        ...
        jLabel13.setText("(1: facile 2: moyen 3: difficile)");
    ...
    private void jManualStartActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jManualStartActionPerformed
        ...
        //on lance l'activité
        LaunchPolygons l = new LaunchPolygons (this, size, durée, level, (Integer) jDifficulty.getValue()) ;
...
class LaunchPolygons extends Thread implements ActionListener, KeyListener {
    ...
    DoubleCadreSimple grilleL, grilleR ;
    Random rand ;
    ...
    LaunchPolygons (JPanel p, int size, int durée, int level, int difficulty) {
        this.p = p ;
        this.size = size ;
        this.durée = durée * 60 ;
        this.level = level ;
        ...
        grilleL = new DoubleCadreSimple (size) ;
        ...
        grilleR = new DoubleCadreSimple (size) ;
    ...
    public void run () {
        ...
        if (results.size() > 0) {
            ...
            UserInfo.resultatsPolygons.add(session);
            UserInfo.modifiedResultatsPolygons = true ;
        }
        ...
        OrthoVS.fen.computeChartsPolygons(true);
        p.setVisible (true) ;
        OrthoVS.fen.repaint () ;
    }
    
    //Générer des polygones
    // voir http://stackoverflow.com/questions/8997099/algorithm-to-generate-random-2d-polygon
    void randomInit (int difficulty) {
        ...
        //1er polygone: On Choisi le nbre de cotés
        max = 6 ; min = 3 ;
        nCotés = rand.nextInt((max - min) + 1) + min;
        grilleL.polygon1 = new GeneralPath(GeneralPath.WIND_EVEN_ODD, nCotés);
        //On le crée
        cX = grilleL.getWidth() / 2 ; cY = grilleL.getHeight() / 2 ;
        grilleL.polygon1 = generatePolygon (nCotés,cX, cY, 0.1, 0.1);
        
        
        //2nd polygone: On Choisi le nbre de cotés
        max = 5 ; min = 3 ;
        nCotés = rand.nextInt((max - min) + 1) + min;
        //Centre du polygone
        cX = grilleL.getWidth() / 2 ; cY = grilleL.getHeight() / 2 ;
        //On le crée
        grilleL.polygon2 = generatePolygon (nCotés,cX, cY, 0.1, 0.1);

        //On choisi le polygone de ref
        if (rand.nextBoolean()) grilleR.polygon1 = grilleL.polygon1 ;
        else grilleR.polygon1 = grilleL.polygon2 ;
        //On le modifie si besoin...
        if (! pareils) grilleR.polygon1 = distordPolygon (grilleR.polygon1, 0.21/difficulty) ;
        
        //on décale les polygones de la grilleL
        grilleL.polygon1 = deplacePolygon (grilleL.polygon1, - (cX / 3)) ;        
        grilleL.polygon2 = deplacePolygon (grilleL.polygon2, cX / 3) ;
        
        /*//2nd polygone: On Choisi le nbre de cotés
        max = 5 ; min = 3 ;
        nCotés = rand.nextInt((max - min) + 1) + min; rY = rand.nextInt((max - min) + 1) + min;
        grilleL.polygon2 = new GeneralPath(GeneralPath.WIND_EVEN_ODD, 4);
        //Zone de dessin
        max = size*50 - 15 ; min = 20 ;
        rX = rand.nextInt((max - min) + 1) + min; rY = rand.nextInt((max - min) + 1) + min;
        grilleL.polygon2.moveTo(rX, rY);
        //On dessine le polygone
        for (int i=1; i < nCotés; i++ ) {
            rX = rand.nextInt((max - min) + 1) + min; rY = rand.nextInt((max - min) + 1) + min;
            grilleL.polygon2.lineTo(rX, rY);
        }
        grilleL.polygon2.closePath();*/

        grilleL.repaint () ; grilleR.repaint () ;
        tempsDebut = System.currentTimeMillis();
    }
    
    //irregularity  : modifie l'angle (=> poly non régulier)
    //spikeyness    : modifie le rayon (=> poly non inscrit)
    GeneralPath generatePolygon (int n, int cX, int cY, double irregularity, double spikeyness) {
        GeneralPath g = new GeneralPath(GeneralPath.WIND_EVEN_ODD, n+1) ;
        double iMin = 1.0 - irregularity ;
        double sMin = 1.0 - spikeyness   ;
        double rangeMax = 1.0 ;
        double rT = iMin + (rangeMax - iMin) * rand.nextDouble();
        double rR = sMin + (rangeMax - sMin) * rand.nextDouble();
        //premier point
        double theta = (2*Math.PI / (double) (n));
        double radius = grilleL.getWidth()/3 - 5  ;
        double t = theta * rT ;
        double r = radius * rR ;
        g.moveTo((double) cX + (r * Math.cos(t)), (double) cY + (r * Math.sin(t)));
        //points suivants
        for (int i=1; i < n; i++ ) {
            rT = iMin + (rangeMax - iMin) * rand.nextDouble();
            rR = sMin + (rangeMax - sMin) * rand.nextDouble();
            //if (rand.nextBoolean()) r = - r ;
            t = t + theta * rT ;
            r = radius * rR ;
            g.lineTo ((double) cX + (r * Math.cos(t)), (double) cY + (r * Math.sin(t)));
        }
        g.closePath();
        return g;
    }
    
    GeneralPath deplacePolygon (GeneralPath g, int delta) {
        PathIterator p = g.getPathIterator(null) ;
        GeneralPath copie = new GeneralPath (GeneralPath.WIND_EVEN_ODD, 4) ;
        
        double[] coord = new double[6] ;
        while (p.isDone() == false) {
            switch (p.currentSegment(coord)) {
                case PathIterator.SEG_MOVETO : copie.moveTo(coord[0]+delta, coord[1]); break ;
                case PathIterator.SEG_LINETO : copie.lineTo(coord[0]+delta, coord[1]); break ;
                case PathIterator.SEG_CLOSE  : copie.closePath();
            }
            p.next();
        }
        return copie ;
    ...
    GeneralPath distordPolygon (GeneralPath g, double irregularity) {
        PathIterator p = g.getPathIterator(null) ;
        GeneralPath copie = new GeneralPath (GeneralPath.WIND_EVEN_ODD, 4) ;
        double[] coord = new double[6] ;
        ...
        nbPts-- ;
        //On choisi le point à modifier :
        int idx = rand.nextInt((--nbPts - 0) + 1) + 0;
        int boucle = 0 ;
        //On reboucle sur le shape
        p = g.getPathIterator(null) ;
        while (p.isDone() == false) {
            switch (p.currentSegment(coord)) {
                case PathIterator.SEG_MOVETO : {
                    if (idx == boucle) {
                        if (rand.nextBoolean()) coord[0] = coord[0] * (1.0 - irregularity ) ;
                        else coord[1] = coord[1] * (1.0 - irregularity ) ;
                    }
                    copie.moveTo(coord[0], coord[1]); break ;
                }
                case PathIterator.SEG_LINETO : {
                    ...
                    copie.lineTo(coord[0], coord[1]);
                } break ;
                case PathIterator.SEG_CLOSE  : copie.closePath(); break ;
            }
            boucle++; p.next(); 
        }
        return copie ; 
...
class DoubleCadreSimple extends JPanel {
    
    int size ;
    boolean pareils = false ;
    public GeneralPath polygon1, polygon2 ;
    
    DoubleCadreSimple ( int size ) {
        this.size = size ;
    ...
    public void paintComponent(Graphics g) {
        ...
        if (polygon1 != null) g2.draw(polygon1);
        if (polygon2 != null) g2.draw(polygon2);
    }
