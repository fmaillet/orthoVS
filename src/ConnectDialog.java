...
public class ConnectDialog extends java.awt.Dialog {
    ...
    private void initComponents() {
        ...
        setResizable(false);
        setTitle("orthoVS : connexion au serveur...");
    ...
    String nom, prenom, titre, message, version, activite, dateLimite ;
     ...
     public boolean connection (String a, String c) {
        ...
        try {
            ...
            if (leResultat.next()) {
                ...
                if ( (jeton & 0x02) != 0) { // le jeton est pris !!
                    ...
                    jeton = (byte) (jeton | 0x02) ; //second bit pour orthoVS
                ...
                if (ok) {
                    ...
                    leResultat.updateString("MACADR", OrthoVS.user.macaddress); // 3ème colonne
        ...
        try {
            transmission = laConnection.createStatement (ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE) ;
            String soft = "EVA" ;
            leResultat = transmission.executeQuery ("select * from Softs where SOFT = \"VS\"") ;
            if (leResultat.next()) {
                version = leResultat.getString("VERSION") ;
                long ver = Long.parseLong(version);
                if (Integer.parseInt(version) > OrthoVS.user.softVersion )
                    jMessages.append ("-> Version logiciel plus récente sur le serveur : pensez à mettre à jour...\n") ;
            }
        ...
        if (nom == null) return false ;
            ...
            OrthoVS.user.adeli = a ;
            OrthoVS.user.code = c ;
            OrthoVS.user.nom = nom ;
            OrthoVS.user.prenom = prenom ;
            OrthoVS.user.titre = titre ;
            OrthoVS.user.activite = activite ;
            jMessages.append ("-> Connexion ok : "+OrthoVS.user.titre+" "+OrthoVS.user.nom+" "+OrthoVS.user.prenom+" ("+OrthoVS.user.activite+")\n") ;
